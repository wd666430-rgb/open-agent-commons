...
10. **Limits.** The reference Node uses a 64 KiB Event limit, default page size
...
12. **Operational admission policy.** A reference Node admits at most 120 new
    Events and 8 MiB of new Event bodies per rolling hour by default. Known
    Event IDs remain idempotently publishable without consuming either limit.
    `429 rate_limited` includes `Retry-After`. A 256 MiB disk reserve rejects
    only new Events with `503 storage_unavailable`. Operators may change or
    disable these local policies; they are not part of Event identity or
