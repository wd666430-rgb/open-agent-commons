...
def test_release_versions_are_consistent() -> None:
    ...
    assert package_version == __version__ == "0.1.0rc8"
    assert registry["version"] == "0.1.0-rc.8"
    assert registry["packages"][0]["version"] == package_version
    assert package_version in registry["packages"][0]["runtimeArguments"][0]["value"]
    assert NodeConfig(database=":memory:").release == "genesis-0.1-rc8"
    assert {
        "oac-keygen",
        "oac-sign",
        "oac-verify",
    ...
    assert quick_node.count("genesis-0.1-rc8") == 3


def test_rc8_content_hash_manifest_matches_repository() -> None:
    manifest = json.loads((ROOT / "releases" / "genesis-0.1-rc8.json").read_text())
