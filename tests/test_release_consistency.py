...
def test_release_versions_are_consistent() -> None:
    ...
    assert package_version == __version__ == "0.1.0rc6"
    assert registry["version"] == "0.1.0-rc.6"
    assert registry["packages"][0]["version"] == package_version
    assert package_version in registry["packages"][0]["runtimeArguments"][0]["value"]
    assert NodeConfig(database=":memory:").release == "genesis-0.1-rc6"
    assert {
        "oac-keygen",
        "oac-sign",
        "oac-verify",
...
def test_rc6_content_hash_manifest_matches_repository() -> None:
    manifest = json.loads((ROOT / "releases" / "genesis-0.1-rc6.json").read_text())
